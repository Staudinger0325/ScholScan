...
def extract_metrics(sample):
    ...
    m = {
        ...
        "location": {
            "gold_steps": int(m.get("location", {}).get("gold_steps", 0) or 0),
            "hit_steps": int(m.get("location", {}).get("hit_steps", 0) or 0),
            "extra_steps": int(m.get("location", {}).get("extra_steps", 0) or 0),
        },
        "reasoning": {
            "gold_steps": int(m.get("reasoning", {}).get("gold_steps", 0) or 0),
            "reached_steps": int(m.get("reasoning", {}).get("reached_steps", 0) or 0),
            "missing_steps": int(m.get("reasoning", {}).get("missing_steps", 0) or 0),
        },
...
def get_error_type(sample):
    t = sample.get("Error Type") or sample.get("error_type") or sample.get("type")
    if not t:
        return None
...
def main():
    ...
    if n_all > 0:
        ...
        print(f"Total Avg Score: {avg_total:.6f}")
    else:
        print("Total Avg Score: N/A")
