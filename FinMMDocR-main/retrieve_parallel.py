...
def process_single_query(example, model_name):
    question_id = example["question_id"]
    question = example["question"]
    texts_path = example["texts"]
    logger.info(f"Processing question_id: {question_id}")
    with open(texts_path, "r") as f:
        texts = json.load(f)
    results = get_retrieved_documents(
        query_text_list=[question], 
        documents=texts, 
        top_k=-1, 
        model_name=model_name
    )
    logger.info(f"Completed question_id: {question_id}")
    return {
...
if __name__ == "__main__":
    model_name = "bm25"
    logger.info(f"Starting retrieval process with model: {model_name}")
    ...
    with ProcessPoolExecutor(max_workers=num_processes) as executor:
        ...
        futures = [
            executor.submit(process_single_query, example, model_name) 
            for example in data
        ]
        
        # Collect results, use tqdm to show overall progress
        with tqdm(total=len(futures), desc="Processing queries") as pbar:
            for future in futures:
                result = future.result()
                retrieved_results.append(result)
                pbar.update(1)
                pbar.set_postfix({'current_id': result['question_id']})

    logger.info(f"Completed processing all queries. Saving results...")
    
    # Save results
    output_path = f"./retrieved_results/{model_name}.json"
